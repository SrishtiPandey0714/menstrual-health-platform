...
// Only initialize Groq if API key is available (optional fallback)
...
async function generateFoodWithGroq(phase, profile) {
    if (!isGroqConfigured) {
        throw new Error('Groq API is not configured. Please add GROQ_API_KEY to environment variables.');
    }
